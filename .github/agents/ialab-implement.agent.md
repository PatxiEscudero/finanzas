...
- **ALWAYS** create a new feature branch for any code changes unless explicitly instructed otherwise if feature branch doesn't exist.
- **NEVER** perform commits, pushes or merges without explicit human consent.
...
- **MANDATORY PLANNED TASK UPDATE**: if based on a plan file under `./.github/features/`, the path to `plan.md` will be provided as input (e.g. `./.github/features/20260312-oic-multi-fabrication/plan.md`). Derive the feature folder directly from the parent directory of that path. Update the task "status" and "date" fields to reflect progress, but no other field **UNDER ANY CIRCUMSTANCE**.
...
- Use instructions from `./.github/instructions/` as the authoritative source for coding style, conventions, and best practices.
- Adhere to the highest standards of software engineering. Every code change must comply with the following principles and quality gates.

### Design Principles (Auto-Applied)

- **SOLID**: Single Responsibility, Open/Closed, Liskov Substitution, Interface Segregation, Dependency Inversion
- **Patterns**: Apply recognized design patterns (Gang of Four) only when solving a real, existing problem. Document the pattern and its rationale in a Decision Record.
